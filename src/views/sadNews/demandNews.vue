<template>
	<div class="box">
		<div class="stay">您现在所在位置:供需信息><span>需求信息</span></div>
		<div class="tsBox">
			<div class="tradesBox">
		...
		<div class="pageBox">
			<div class="ulBox">
				<div v-for="(item,index) in unityList" :key="index" @click="toNeedList(item)" @mouseover="destroyTime">
						...
						if (this.unityList.length != 0) {
							for (let i in this.unityList) {
								if (this.unityList[i].detail) {
									this.unityList[i].detail = this.$globalMethod.showHtml(this.unityList[i].detail)
								}
